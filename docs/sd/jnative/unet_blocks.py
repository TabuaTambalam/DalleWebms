...
class ResnetBlock(nn.Module):
    def __init__(
        ...
        super().__init__()
        self.in_layers_0 = nn.GroupNorm(num_groups=32,num_channels=chn_in,eps=0.000010)
        ...
        self.out_layers_0 = nn.GroupNorm(num_groups=32,num_channels=chn_out,eps=0.000010)
...
class AttentionBlock(nn.Module):
    def __init__(
        ...
        self.norm = nn.GroupNorm(affine=True, eps=0.000001, num_channels=channels, num_groups=32)
        self.proj_in = nn.Conv2d(bias=True, dilation=(1,1), groups=1, in_channels=channels, kernel_size=(1,1), out_channels=channels, padding=(0,0), padding_mode=config.pad, stride=(1,1))
        self.transformer_blocks_0_norm1 = nn.LayerNorm(elementwise_affine=True, eps=0.000010, normalized_shape=(channels,))
        self.transformer_blocks_0_attn1 = nn.MultiheadAttention(channels, 8,batch_first=True)
        self.transformer_blocks_0_norm2 = nn.LayerNorm(elementwise_affine=True, eps=0.000010, normalized_shape=(channels,))
        self.transformer_blocks_0_attn2 =  nn.MultiheadAttention(channels, 8,kdim=768,vdim=768,batch_first=True)
        self.transformer_blocks_0_norm3 = nn.LayerNorm(elementwise_affine=True, eps=0.000010, normalized_shape=(channels,))
    ...
    def forward(self, x, cond_k,cond_v):
        ...
        h=int(x.size(2))
        w=int(x.size(3))
        v_184 = self.proj_in(self.norm(x)).permute(0,2,3,1).reshape(b, -1, self.chn)
        v_185 = self.transformer_blocks_0_norm1(v_184)

        v_206 = v_184 + self.transformer_blocks_0_attn1(v_185,self.s_attnK(v_185),self.s_attnV(v_185),need_weights=False)[0]
        v_228 = v_206 + self.transformer_blocks_0_attn2(self.transformer_blocks_0_norm2(v_206),self.c_attnK(cond_k),self.c_attnV(cond_v),need_weights=False)[0]
